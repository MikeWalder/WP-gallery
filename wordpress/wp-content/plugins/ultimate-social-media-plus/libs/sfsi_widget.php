	...
	function sfsi_plus_check_visiblity($isFloter = 0, $share_url = null, $container_position = 'absolute', $shortcode_listener = true)
	{
		...
		function sfsi_plus_prepairIcons($icon_name, $is_front = 0, $onpost = "no", $fromPost = NULL, $share_url = null)
		{
			...
			if (isset($icon) && !empty($icon) && filter_var($icon, FILTER_VALIDATE_URL)) {
				$icons .= "<div style='width:" . $icon_width . "px; height:auto;margin-left:" . $icons_space . "px;margin-bottom:" . $margin_bot . "' class='sfsi_plus_wicons shuffeldiv " . $cmcls . "'>";

				$icons .= "<div class='sfsiplus_inerCnt'>";

				$icons .= "<a class='" . $class . " sficn' data-effect='" . $mouse_hover_effect . "' $new_window  href='" . $url . "'  style='width:" . $icon_width . "px; height:" . $icon_width . "px;opacity:" . $icon_opacity . ";".$sfsi_plus_icon_bgColor_style."' " . (isset($sfsi_onclick) ? 'onclick="' . $sfsi_onclick . '"' : '') . ">";
